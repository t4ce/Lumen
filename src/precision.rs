...
pub fn precision_guard(config: PrecisionConfig) -> PrecisionConfigGuard {
    let lock = GLOBAL_CONFIG_SCOPE_DEPTH.with(|depth| {
        let current = depth.get();
        depth.set(current + 1);
        if current == 0 {
            Some(
                PRECISION_CONFIG_LOCK
                    .lock()
                    .expect("precision config lock poisoned"),
            )
        } else {
            None
        }
...
impl Drop for PrecisionConfigGuard {
    fn drop(&mut self) {
        ...
        GLOBAL_CONFIG_SCOPE_DEPTH.with(|depth| {
            let current = depth.get();
            debug_assert!(current > 0, "precision scope depth underflow");
...
impl Drop for ParameterQuantizationGuard {
    fn drop(&mut self) {
        ...
        GLOBAL_CONFIG_SCOPE_DEPTH.with(|depth| {
            let current = depth.get();
            debug_assert!(current > 0, "parameter quantization scope depth underflow");
            depth.set(current.saturating_sub(1));
        });
    }
}
