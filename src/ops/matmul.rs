...
fn matvec_rowmajor_rowwise_parallel_f32_bf16(
    ...
    out.par_iter_mut().enumerate().for_each(|(i, out_val)| {
        let row = &w_rowmajor[i * k_dim..(i + 1) * k_dim];
        *out_val = dot_unrolled_f32_bf16(x, row);
    });
}
...
fn matvec_rowmajor_rowwise_parallel_f32_f16(
    ...
    out.par_iter_mut().enumerate().for_each(|(i, out_val)| {
        let row = &w_rowmajor[i * k_dim..(i + 1) * k_dim];
        *out_val = dot_unrolled_f32_f16(x, row);
    });
}
...
pub fn matvec_rowmajor_parallel_i8_i8(
    ...
    with_i8_input_as_f32(x, x_scale, |x_f32| {
        matvec_rowmajor_parallel_f32_i8(x_f32, w_rowmajor, w_scale, n_rows, k_dim, out);
    });
}
...
#[inline]
fn dual_matvec_silu_mul_rowmajor_parallel_i8_i8(
    x: &[i8],
    x_scale: f32,
    gate_w_rowmajor: &[i8],
    gate_scale: f32,
...
fn matvec_argmax_rowmajor_parallel_i8_i8(
    ...
    with_i8_input_as_f32(x, x_scale, |x_f32| {
        matvec_argmax_rowmajor_parallel_f32_i8(x_f32, w_rowmajor, w_scale, n_rows, k_dim)
    })
}
